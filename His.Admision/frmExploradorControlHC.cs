    ...
    public partial class frmExploradorControlHC : Form
    {
        ...
        private void toolStripButtonActualizar_Click(object sender, EventArgs e)
        {
                ...
                Nhc.Checked == false && chkEstado.Checked == false) //Valida solo la fecha
            {
                ...
                {
                    ultraGridControl.DataSource = Negocio.PorFecha(Convert.ToString(dtpFiltroDesde.Value),Convert.ToString(dtpFiltroHasta.Value));
                    RedimencionarTabla();
                ...
                Nhc.Checked == true && chkEstado.Checked == false) //valida por fecha y HC del paciente
            {
                if (dtpFiltroDesde.Value.Date < dtpFiltroHasta.Value.Date)
                {
                    ultraGridControl.DataSource = Negocio.PorFechayHc(Convert.ToString(dtpFiltroDesde.Value), Convert.ToString(dtpFiltroHasta.Value), txthc.Text);
                    RedimencionarTabla();
                    if(ultraGridControl.Rows.Count <= 0)
                    {
                        MessageBox.Show("No Hay Registros Para Mostrar", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                Nhc.Checked == false && chkEstado.Checked == true) //Valida por fecha y por estado 
            {
                if (dtpFiltroDesde.Value.Date < dtpFiltroHasta.Value.Date)
                {
                    ...
                    {
                        ultraGridControl.DataSource = Negocio.PorFechayEstado(Convert.ToString(dtpFiltroDesde.Value), Convert.ToString(dtpFiltroHasta.Value), cboEstado.SelectedText);
                        RedimencionarTabla();
                        if (ultraGridControl.Rows.Count <= 0)
                        {
                            MessageBox.Show("No Hay Registros Para Mostrar", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                Nhc.Checked == true && chkEstado.Checked == true) //valida por fecha, hc y estado
            {
                if (dtpFiltroDesde.Value.Date < dtpFiltroHasta.Value.Date)
                {
                    if (cboEstado.SelectedIndex != -1)
                    {
                        ultraGridControl.DataSource = Negocio.PorFechaHCEstado(Convert.ToString(dtpFiltroDesde.Value), Convert.ToString(dtpFiltroHasta.Value), txthc.Text, cboEstado.SelectedText);
                        RedimencionarTabla();
                        if (ultraGridControl.Rows.Count <= 0)
                        {
                            MessageBox.Show("No Hay Registros Para Mostrar", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                Nhc.Checked == false && chkEstado.Checked == false)
            {
                if (dtpFiltroDesde.Value.Date < dtpFiltroHasta.Value.Date)
                {
                    ultraGridControl.DataSource = Negocio.PorFechaAlta(Convert.ToString(dtpFiltroDesde.Value), Convert.ToString(dtpFiltroHasta.Value));
                    RedimencionarTabla();
                    if (ultraGridControl.Rows.Count <= 0)
                    {
                        MessageBox.Show("No Hay Registros Para Mostrar", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                Nhc.Checked == true && chkEstado.Checked == false) //valida por fecha alta y HC del paciente
            {
                if (dtpFiltroDesde.Value.Date < dtpFiltroHasta.Value.Date)
                {
                    ultraGridControl.DataSource = Negocio.PorFechaAltayHc(Convert.ToString(dtpFiltroDesde.Value), Convert.ToString(dtpFiltroHasta.Value), txthc.Text);
                    RedimencionarTabla();
                    if (ultraGridControl.Rows.Count <= 0)
                    {
                        MessageBox.Show("No Hay Registros Para Mostrar", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                Nhc.Checked == false && chkEstado.Checked == true) //Valida por fecha y por estado 
            {
                if (dtpFiltroDesde.Value.Date < dtpFiltroHasta.Value.Date)
                {
                    if (cboEstado.SelectedIndex != -1) //controla que en el combo no se eliga vacio
                    {
                        ultraGridControl.DataSource = Negocio.PorFechaAltayEstado(Convert.ToString(dtpFiltroDesde.Value), Convert.ToString(dtpFiltroHasta.Value), cboEstado.SelectedText);
                        RedimencionarTabla();
                        if (ultraGridControl.Rows.Count <= 0)
                        {
                            MessageBox.Show("No Hay Registros Para Mostrar", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                Nhc.Checked == true && chkEstado.Checked == true) //valida por fecha alta, hc y estado
            {
                if (dtpFiltroDesde.Value.Date < dtpFiltroHasta.Value.Date)
                {
                    if (cboEstado.SelectedIndex != -1)
                    {
                        ultraGridControl.DataSource = Negocio.PorFechaAltaHCEstado(Convert.ToString(dtpFiltroDesde.Value), Convert.ToString(dtpFiltroHasta.Value), txthc.Text, cboEstado.SelectedText);
                        RedimencionarTabla();
                        if (ultraGridControl.Rows.Count <= 0)
                        {
                            MessageBox.Show("No Hay Registros Para Mostrar", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
