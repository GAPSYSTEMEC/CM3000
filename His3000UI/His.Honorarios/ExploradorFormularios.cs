...
namespace His.Honorarios
{
    public partial class frm_ExploradorFormularios : Form
    {
        ...
        private void toolStripButtonActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ultraGridPacientes.DataSource = Negocio.NegPacientes.getAtencionesFormularios(dtpFiltroDesde.Value.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss"), dtpFiltroHasta.Value.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss"), chkIngreso.Checked, chkAlta.Checked, chkFacturacion.Checked, chbTipoIngreso.Checked, Convert.ToInt32(cboTipoIngreso.SelectedValue), chkTratamiento.Checked, Convert.ToInt32(cmb_tipoatencion.SelectedValue), chkHC.Checked, Convert.ToInt32(txt_historiaclinica.Text),  chkFormulario.Checked, cmbFormulario.Text.Trim());
            }
            catch (Exception err) { MessageBox.Show(err.Message); }
        }
        ...
        private void ultraGridPacientes_DoubleClickCell(object sender, Infragistics.Win.UltraWinGrid.DoubleClickCellEventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    ...
                    {
                        His.Formulario.Consulta consulta = new Formulario.Consulta(codigoAtencion, ultraGridPacientes.ActiveRow.Cells["HC"].Value.ToString(), false);
