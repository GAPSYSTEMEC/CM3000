    ...
    public partial class frmQuirofanoPedidoPaciente : Form
    {
        ...
        public void CargarProcedimientoPaciente()
        {
           DataTable dt = new DataTable();
            dt = Quirofano.ProcedimientosPaciente(ate_codigo, datosPaciente.PAC_CODIGO.ToString());
        ...
        public void GuardarCantidades()
        {
            ...
            try
            {
                ...
                //else
                // {
                //     //MessageBox.Show("No tiene stock disponible.\r\nComunise con el administrador.", "HIS3000", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                // }
        ...
        {
            //frm_QuirofanoNuevo x = new frm_QuirofanoNuevo();
            //x.cie_codigo = cie_codigo;
            ...
            {
                frm_QuirofanoAgregarVarios x = new frm_QuirofanoAgregarVarios();
                x.ate_codigo = ate_codigo;
                x.pac_codigo = pac_codigo;
        ...
        private void X_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (codigo_producto != null && orden != null && cant != null)
            {
                try
                {
                    ...
                    DataTable TablaProcedimientos = Quirofano.PacienteProcedimiento(cie_codigo, ate_codigo);
                    bool existe = false;
        ...
        private void toolStripCerrar_Click(object sender, EventArgs e)
        {
                ...
                "His3000", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
            {
                try
                {
                    ...
                    TablaPro = Quirofano.PacienteProcedimiento(cie_codigo, ate_codigo);
                    bool error = false; //Verifica si existe stock disponible para pedido adicional, si es falso hace el pedido caso contrario no lo hara.
                    foreach (DataRow item in TablaPro.Rows)
                    {
                        if(Convert.ToInt32(item[3].ToString()) >=
                            Convert.ToInt32(item[4].ToString()))
                        {
                            error = false;
                        }
                        else
                    ...
                    //for (int i = 0; i < TablaPro.Rows.Count; i++)
                    //{
                    //    if (Convert.ToInt32(TablaPro.Rows[i].Cells[3].Value.ToString()) >=
                    //        Convert.ToInt32(TablaProcedimientos.Rows[i].Cells[4].Value.ToString()))
                    //    {
                    //        error = false;
                    //    }
                    //    else
                    ...
                    if (error == false) //if (PedidoAdicional == true && error == false)
                    {
                        try
                        {
                            ...
                            foreach (DataRow item in TablaPro.Rows)
                            {
                                ...
                                codpro = item[0].ToString();
                                cant_usada = item[4].ToString();
                                Quirofano.PedidoAdicional(ate_codigo, pac_codigo, cie_codigo, cant_usada, codpro); //Aqui graba cuando ya se decida hacer el pedido adicional

                                //PEDIDO ADICION -- CUENTA PACIENTE
                                if (Convert.ToInt32(item[4].ToString()) > 0)
                                {
                                    PedidosDetalleItem = new PEDIDOS_DETALLE();
                                    codpro = item[0].ToString();
                                    DataTable TablaP = NegProducto.RecuperarProductoSic(codpro);
                                    pro_descripcion = item[1].ToString();
                                    cantidad = item[4].ToString();
                                    orden = item[8].ToString();
                                    costo = Convert.ToDouble(item[10].ToString());
                                    total = Math.Round((Convert.ToInt32(cantidad) * costo), 2);

                                    //Cambios Edgar 20210601
                                    string xIVA = "1." + TablaP.Rows[0]["iva"].ToString(); //SE REGRESA EL VALOR UNITARIO DEL PRECIO DE VENTA
                                    double valorsinIVA = Math.Round(Convert.ToDouble(item[10].ToString()) / Convert.ToDouble(xIVA),2);

                                    PedidosDetalleItem.PDD_CODIGO = 1;
                                    PedidosDetalleItem.PRODUCTOReference.EntityKey = Prod.EntityKey;
                                    PedidosDetalleItem.PDD_CANTIDAD = Convert.ToInt32(item[4].ToString());
                                    PedidosDetalleItem.PRO_DESCRIPCION = item[1].ToString();
                                    PedidosDetalleItem.PDD_VALOR = Convert.ToDecimal(valorsinIVA);
                                    PedidosDetalleItem.PDD_IVA = Math.Round(((((Convert.ToDecimal(valorsinIVA) * Convert.ToDecimal(item[4].ToString()))) * (Convert.ToDecimal(TablaP.Rows[0]["iva"].ToString()))) / 100), 4);
                                    PedidosDetalleItem.PDD_TOTAL = Math.Round((Convert.ToDecimal(valorsinIVA) * Convert.ToDecimal(item[4].ToString())), 2) + Math.Round(((((Convert.ToDecimal(valorsinIVA) * Convert.ToDecimal(item[4].ToString()))) * (Convert.ToDecimal(TablaP.Rows[0]["iva"].ToString()))) / 100), 2);
                                    PedidosDetalleItem.PDD_ESTADO = true;
                                    PedidosDetalleItem.PDD_COSTO = 0;
                                    PedidosDetalleItem.PDD_FACTURA = null;
                                    PedidosDetalleItem.PDD_ESTADO_FACTURA = null;
                                    PedidosDetalleItem.PDD_FECHA_FACTURA = null;
                                    PedidosDetalleItem.PDD_RESULTADO = null;
                                    PedidosDetalleItem.PRO_CODIGO_BARRAS = item[0].ToString();
            ...
            try
            {
                ...
                if (PedidosDetalle.Count() > 0)
                {
                    ...
                    var pedido = new PEDIDOS
                    {
                        ...
                        PED_DESCRIPCION = "PEDIDO GENERADO DESDE QUIROFANO.",
                    ...
                    foreach (var solicitud in PedidosDetalle)
                    {
                        ...
                        if (codigoArea != 1)
                        {
                            ...
                            //Valida duplicados
                                ...
                                NegPedidos.crearDetallePedidoQuirofano(solicitud, pedido, XRubro, xcodDiv, "");
                                //Bodega despacha de la 12
                                double bodega = NegParametros.ParametroBodegaQuirofano();
                                ...
                                NegQuirofano.ActualizarKardexSic(pedido.PED_CODIGO.ToString());
                ...
                if (validador)
                {
                    ...
                    if (x.ped_codigo != null)
                    {
                        ...
                        if (DevolucionNumero != 0)
                        {
                            ...
                            //ACTUALIZA DENTRO DEL KARDEX EL NUMERO DE LA BODEGA
                            NegQuirofano.ActualizarKardexSic(DevolucionNumero.ToString());
                            CargarProcedimientosPaciente();
                        }
                        //else
                        //{
        ...
        private void btnactualizar_Click(object sender, EventArgs e)
        {
            if(MessageBox.Show("¿Desea eliminar productos con cantidad en cero?", "HIS3000", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                for (int i = 0; i < TablaProcedimientos.Rows.Count; i++)
                {
                    if(Convert.ToInt32(TablaProcedimientos.Rows[i].Cells[4].Value.ToString()) <= 0)
                    {
                        NegQuirofano.QuirofanoEliminaRegistro(TablaProcedimientos.Rows[i].Cells[0].Value.ToString(),
               Convert.ToInt32(cie_codigo), Convert.ToInt64(ate_codigo));
                    }
                }
                CargarProcedimientoPaciente();
            ...
            if (e.ColumnIndex == 4)
            {
                ...
                else
                {
                    NegQuirofano.QuirofanoActualizaProductos(TablaProcedimientos.Rows[rowAnterior].Cells[0].Value.ToString(),
                        Convert.ToInt32(cie_codigo), Convert.ToInt32(TablaProcedimientos.Rows[rowAnterior].Cells[4].Value.ToString()), Convert.ToInt64(ate_codigo));
                }
            }
        }
    }
