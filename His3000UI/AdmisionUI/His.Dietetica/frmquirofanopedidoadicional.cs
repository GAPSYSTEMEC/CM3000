...
namespace His.Dietetica
{
    public partial class frmquirofanopedidoadicional : Form
    {
        ...
        public void CargarTickets()
        {
            try
            {
                TablaTickets.DataSource = Quirofano.VerTicketsPaciente(ultimaAtencion.ATE_CODIGO.ToString());
            }
            catch (Exception ex)
            {
        ...
        public void GuardarCambios()
        {
            ...
            if(TablaPedidos.Rows.Count > 0)
            {
                try
                {
                    DataTable TablaProcedimientos = Quirofano.PacienteProcedimiento(cie_codigo, ultimaAtencion.ATE_CODIGO.ToString());
        ...
        public void GuardarPedido()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow item in TablaPedidos.Rows)
                {
                    ...
                    PedidosDetalleItem.PDD_CANTIDAD = Convert.ToInt32(item.Cells[2].Value.ToString());
                    ...
                    foreach (var solicitud in PedidosDetalle)
                    {
                        ...
                        if (codigoArea != 1)
                        {
                            ...
                            NegQuirofano.DatosReposicion(Convert.ToInt64(ate_codigo), Convert.ToInt32(cie_codigo),
                                Convert.ToInt32(solicitud.PDD_CANTIDAD), Convert.ToDateTime(DateTime.Now.ToString()),
                ...
                if (txtcantidad.Text.Trim() != "")
                {
                    ...
                    if (Convert.ToInt32(Fila.Cells["STOCK"].Value.ToString()) != 0 && Convert.ToInt32(Fila.Cells["STOCK"].Value.ToString()) >= Convert.ToInt32(txtcantidad.Text))
                    {
                        ...
                        foreach (DataGridViewRow item in TablaPedidos.Rows)
                        {
                            ...
                            //    foreach (DataRow item1 in TablaProcedimientos.Rows)
                            //    {
                            //        if (item1["Codigo"].ToString() == codpro)
                            //        {
                            //            Quirofano.ActualizarPedidoAdicional(ultimaAtencion.ATE_CODIGO.ToString(), cie_codigo, codpro, txtcantidad.Text);
                            //            MessageBox.Show("El producto ya está agregado.", "HIS3000", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            //            existe = true;
                            //            break;
                            //        }
