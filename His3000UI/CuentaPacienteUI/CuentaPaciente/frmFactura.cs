...
namespace CuentaPaciente
{
    public partial class frmFactura : Form
    {
        ...
        private void cargarDatos()
        {
            ...
            habiltarBotones(true, false, false, false, false, false, true);
        ...
        private void limpiarCampos()
        {
            ...
            habiltarBotones(true, false, false, false, false, true, true);
        ...
        private void habiltarBotones(Boolean nuevo, Boolean guardar, Boolean modificar, Boolean buscar, Boolean imprimir, Boolean cancelar, Boolean salir)
        {
        ...
        private void cargarDetalleFactura()
        {
            ...
            if (VectorCodigoOK == "0")
            {
                ...
                lista = NegDetalleCuenta.RecuperaCuentaPacinteSP();
                NegFactura.EliminaAuxAgrupa();
            }
            else
            {
                ...
                for (int i = 0; i < listaRubros.Count; i++)
                {
                    ...
                    for (int j = 0; j < lista.Rows.Count; j++)
                    {
                        if (Convert.ToInt16(rubros.RUB_CODIGO) == Convert.ToInt16(lista.Rows[j]["RUB_CODIGO"].ToString()))
                        {
                            ...
                            if (Convert.ToDouble(lista.Rows[j]["CUE_IVA"].ToString()) > Convert.ToDouble(0))
                            {
                                iva += Convert.ToDouble(lista.Rows[j]["CUE_IVA"].ToString());
        ...
        private void cargarTotales(int fila)
        {
            try
            {
                if (gridFormasPago.Rows.Count == 2 && gridFormasPago.CurrentRow.Index == 0)
                ...
                else
                {
                        ...
                        if (Convert.ToString(gridFormasPago.Rows[fila].Cells[0].Value) == "15")
                        {
                            if (form1.validador == 0)
                            {
                                ...
                                if (form1.cod_anticipo != null && validaAnticipos)
                                {
                                    ...
                                    gridFormasPago.Rows[fila].Cells[9].Value = form1.nombre_paciente;
                                    gridFormasPago.Rows[fila].Cells[5].Value = 0;
                                    gridFormasPago.Rows[fila].Cells[4].Value = "N-D";
                    ...
                    if (gridFormasPago.CurrentCell.ColumnIndex == 0)
                    {
                        ...
                        label39.Text = (total - valor).ToString();
                                        ...
                                        if (esc_codigo == "2")
                                        {
                                            if (accionPaciente == false)
                                            {
                                                ...
                                                if (facturaPrefactura == 1)
                                                    GuardaFacturaSic3000(ref ParaCodigo);
                                                ...
                                                habiltarBotones(true, false, true, true, true, true, true);
        ...
        public void GeneraFacturaElectronica(ref bool TipoDocumento)
        {
            ...
            if (cajaEmite.Rows[0][0].ToString() == "1")
            {
                ...
                FechasPaciente = NegFactura.FechaPaciente(Convert.ToInt16(txt_Atencion.Text));
                ...
                if (Parametros.Rows[2][2].ToString() == "True")
                {
                    ...
                    string fecha = FechasPaciente.Rows[0][4].ToString();
                    fecha = fecha.Substring(0, 10);
                    cadena = fecha.Substring(0, 2) + fecha.Substring(3, 2) + fecha.Substring(6, 4) +
                        numctrl + Empresa.Rows[0][3].ToString() + Parametros.Rows[0][3].ToString() + "001" + txt_Factura_Cod1.Text
                    ...
                    txtfacturaelectronica.AppendText("  <estab>001</estab>\r\n");

                    txtfacturaelectronica.AppendText("  <ptoEmi>" + txt_Factura_Cod1.Text + "</ptoEmi>\r\n");
                    ...
                    if (verificaResoluciones.Rows.Count > 0)
                    {
                        if (verificaResoluciones.Rows[1][0].ToString() == "True")
                            txtfacturaelectronica.AppendText("  <regimenMicroempresas>" + verificaResoluciones.Rows[1][1].ToString() + "</regimenMicroempresas>\r\n");
                    ...
                    txtfacturaelectronica.AppendText("      <campoAdicional nombre=\"Teléfono Clínica\">" + Empresa.Rows[0][25].ToString() + "</campoAdicional>\r\n");
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            ...
            {
                habiltarBotones(false, true, false, true, false, true, true);
        ...
        private void btnBuscarDatos_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (pacienteNuevo == false)
                {
                    ...
                    {
                        txtNumeroAtencion.Text = NegFactura.RecuperaNumeroAtencion(Convert.ToInt16(txt_Atencion.Text));
                    ...
                    if (txt_Historia_Pc.Text.Trim() != "")
                    {
                        ...
                        {
                            //DateTime fecha = Convert.ToDateTime("2021/05/21");
                            //List<CUENTAS_PACIENTES> lista = new List<CUENTAS_PACIENTES>();
                            //lista = NegFactura.RecuepraCuenta(ultimaAtencion.ATE_CODIGO);
                            //foreach (var item in lista)
                            //{
                            //    if (item.CUE_FECHA <= fecha)
                            //    {
                            //        NegFactura.ArreglaIVA(Convert.ToString(ultimaAtencion.ATE_CODIGO), item.CUE_CODIGO, item.PRO_CODIGO);
                            //    }
                            //}
                            txtObserva.Enabled = true;
        ...
        private void FacturaNombre(string dato)
        {
            ...
            if (cbx_FacturaNombre.SelectedIndex > -1)
            {
                if (dato == "PACIENTE")
                {
                    txt_Ruc.Text = pacienteFactura.PAC_IDENTIFICACION;
                    txt_Direc_Cliente.Text = datosPacienteActual.DAP_DIRECCION_DOMICILIO;
                    txt_Cliente.Text = pacienteFactura.PAC_APELLIDO_PATERNO + " " + pacienteFactura.PAC_APELLIDO_MATERNO + " " + pacienteFactura.PAC_NOMBRE1 + " " + pacienteFactura.PAC_NOMBRE2;
                    txt_telef_Cliente.Text = pacienteFactura.PAC_REFERENTE_TELEFONO;
                    estado = true;
                    btnBuscaClienteSic.Visible = false;
                }
                else if (dato == "ACOMPAÑANTE")
                {
                    if (ultimaAtencion.ATE_ACOMPANANTE_CEDULA != "")
                    {
                        ...
                        if (txt_Ruc.Text == "")
                        {
                            MessageBox.Show("Datos De ACOMPAÑANTE No Fueron Ingresados En La ADMISION", "HIS3000", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                }
        ...
        {
            DataTable OtroCliente = new DataTable();
            if (NegValidaciones.esCedulaValida(txt_Ruc.Text) == true || chk_pasaporte.Checked == true)
            {
                if (chk_pasaporte.Checked == true)
        ...
        private Boolean VerificaNumeroFactura(string NumeroFactura, string NumeroCaja)
        {
            ...
            if (DatosFactura.Rows.Count > 0)
            {
                ...
                txt_Autorizacion.Text = DatosFactura.Rows[0]["numaut"].ToString();
                return true;
            }
            else
            {
        ...
        private void CargaDetalleFacturaCopago(String Local, String CodigoNominaUsuario, int i)
        {
            ...
            if (Convert.ToDecimal(gridDetalleFactura.Rows[i].Cells["TOTAL CON IVA"].Value.ToString()) != 0 && Convert.ToDecimal(gridDetalleFactura.Rows[i].Cells["TOTAL SIN IVA"].Value.ToString()) != 0)
            {
                ...
                DetalleFactura.fechaven = Convert.ToString(DateTime.Now.Year + DateTime.Now.Month + DateTime.Now.Day);
        ...
        private void GuardaFacturaSic3000(ref bool ParaCodigo)
        {
            ...
            FacturaSic3000.fecha = Convert.ToDateTime(DateTime.Now);
            FacturaSic3000.hora = DateTime.Now.TimeOfDay.ToString();
            ...
            FacturaSic3000.fecven = DateTime.Now;
            ...
            for (Int16 i = 0; i < gridFormasPago.Rows.Count; i++)
            {
                if (gridFormasPago.Rows[i].Cells[0].Value != null)
                {
                    ...
                    if (Convert.ToBoolean(FormaPago.Rows[0]["CXC"]) == false)
                    {
                        ...
                        FacturaPago.fecha = DateTime.Now;
                        FacturaPago.fecha1 = DateTime.Now.ToString();
                        FacturaPago.banco = "";
                    ...
                    else
                    {
                        ...
                        FacturaPago.codcli = "0";
                        EstadoCuenta.fecha = DateTime.Now.ToString();
                        ...
                        CuentasxCobrar.fecha = Convert.ToDateTime(string.Format(("{0:dd/MM/yyyy}"), DateTime.Now.ToString()));
                        ...
                        CuentasxCobrar.fecha1 = DateTime.Now.ToString();
        ...
        private void gridDetalleFactura_KeyUp_1(object sender, KeyEventArgs e)
        {
        ...
        private void CargaPaciente()
        {
            ...
            if (listaProductosSolicitados != null && listaProductosSolicitados.Count > 0)
            {
                ...
                foreach (var solicitud in listaProductosSolicitados)
                {
                    ...
                    {
                        int codrubro = NegRubros.RecuperaMaximoRubro();

                        //cambios Edgar 20210205
                        if (solicitud.PRO_DESCRIPCION == "HONORARIOS MEDICOS")//pruebas para agregar medicos
                        {
                            foreach (var item in form.DetalleHonorarios)
                            {
                                ...
                                if (cmb_Areas.Text == "TODAS LAS AREAS")
                                {
                                    ...
                                    NegPedidos.GuardaPedDetalleHonorarios(pdd_codigo, Pedido, item.CODPRO,
                                        "HONORARIOS MEDICOS", Convert.ToDecimal(solicitud.PDD_CANTIDAD), item.VALOR, Convert.ToDecimal(solicitud.PDD_IVA),
                                        item.VALOR, pedido, XRubro, xcodDiv, item.numdoc);
                                ...
                                else
                                {
                                    NegPedidos.GuardaPedDetalleHonorarios(pdd_codigo, Pedido, item.CODPRO,
                                        ...
                                        item.VALOR, pedido, Rubro, Convert.ToInt32(ar.DIV_CODIGO), item.numdoc);
                        ...
                        {
                            if (Convert.ToInt32(ar.PEA_CODIGO) == 16 || Convert.ToInt32(cmb_Rubros.SelectedValue) == 16)
                            {
                                DtoDetalleHonorariosMedicos HonorariosMedico = new DtoDetalleHonorariosMedicos();
                            ...
                            else if (solicitud.PRO_DESCRIPCION == "HONORARIOS MEDICOS") //Cambios Edgar 20210205 quemado para pruebas
                            {
                                DtoDetalleHonorariosMedicos HonorariosMedico = new DtoDetalleHonorariosMedicos();
                                HonorariosMedico = form.DetalleHonorarios.FirstOrDefault(h => h.CODPRO == solicitud.PRODUCTOReference.EntityKey.EntityKeyValues[0].Value.ToString());
                                if (HonorariosMedico != null)
        ...
        private void facturaElectronicaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (ultimaAtencion.ESC_CODIGO == 6)
            {
                ...
                {
                    //if (gridRecuperaFP.Rows.Count > 0)
                    //{
                    ...
                    //}
                    //else
                    //    MessageBox.Show("NO SE PUEDE GENERAR FACTURA ELECTRONICA SIN GUARDAR LA FACTURA", "HIS3000", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
        ...
        private void chbCopago_CheckedChanged(object sender, EventArgs e)
        {
            if (chbCopago.Checked)
            {
                if (VerificaAltaPaciente())
                {
                    ...
                    NegFactura.ActualizaDescuentoAtencion(Convert.ToInt16(txt_Atencion.Text));//encerar descuentos
                                                                                              //alex////////////////////////            
                    cargarDetalleFactura();
                    /////////////////////////////
        ...
        private void btnAgrupar_Click(object sender, EventArgs e)
        {
            ...
            label50.Visible = true;
            AgrupacionCuentasPacientes frm = new AgrupacionCuentasPacientes();
        ...
        public bool VerificaAltaPaciente()
        {
            ...
            ok = NegFactura.VerificaAltaPaciente(Convert.ToInt16(txt_Atencion.Text));
            ...
            try
            {
                ...
                if (ultimaAtencion.ESC_CODIGO == 2)
                {
                    if (codigo_hab != 0)
                    {
                        ...
                        {
                            if (MessageBox.Show("¿Está seguro de HABILITAR (Desbloquear) la cuenta del paciente?",
                    ...
                    else
                    {
                        if (MessageBox.Show("¿Está seguro de HABILITAR (Desbloquear) la cuenta del paciente?",
                           "HIS3000", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
                        {
                            NegFactura.RevertirEscCodigo(Convert.ToInt16(txt_Atencion.Text));
                            DataTable Revertir = NegHabitaciones.RevertirMovimientoHabitacion(objHabitacion);
                            if (Revertir != null)
                            {
                                DataTable InformacionPaciente = new DataTable();
