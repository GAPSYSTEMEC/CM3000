...
namespace CuentaPaciente
{
    public partial class frmCuentaDetalleAtencion : Form
    {
        ...
        public frmCuentaDetalleAtencion()
        {
            InitializeComponent();
        ...
        public bool ValidarCamposCuentas()
        {
            try
            {
                ...
                //    validoCampo = false;
                //}
                if (txtCodigoSic.Text == string.Empty)
                {
        ...
        public void CargarProducto()
        {
            ...
            if (producto.Rows.Count > 0)
            {
                ...
                txtValorNeto.Text = producto.Rows[0][1].ToString();
                txtCantidad.Text = "1";
                txtTotal.Text = (Convert.ToDecimal(txtValorNeto.Text) * Convert.ToDecimal(txtCantidad.Text)).ToString();
        ...
        public void calcularValoresFactura()
        {
            ...
            try
            {
                if (txtValorNeto.Text != string.Empty && txtPorcentaje.Text != string.Empty)
                {
                    ...
                    Valor = String.Format("{0:0.000}", Math.Round((Convert.ToDecimal((Convert.ToDouble(txtCantidad.Text) * Convert.ToDouble(txtValorNeto.Text.ToString())) * porcentaje)), 3));
        ...
        private void ActualizarFilaProducto(CUENTAS_PACIENTES cuentaP)
        {
            try
            {
                ugrdCuenta.ActiveRow.Cells[4].Value = txt_Codigo.Text;
                ugrdCuenta.ActiveRow.Cells[4].Value = txtCodigoSic.Text;
                ugrdCuenta.ActiveRow.Cells[5].Value = txt_Nombre.Text;
                ugrdCuenta.ActiveRow.Cells[6].Value = txtValorNeto.Text;
                ugrdCuenta.ActiveRow.Cells[7].Value = txtCantidad.Text;
                ugrdCuenta.ActiveRow.Cells[8].Value = txtTotal.Text;
                ugrdCuenta.ActiveRow.Cells[9].Value = cuentaP.CUE_IVA.ToString();
                //ugrdCuenta.ActiveRow.Cells[9].Value = cuentaP.CUE_IVA.ToString();
                ugrdCuenta.ActiveRow.Cells[3].Value = dtpFechaPedido.Value;
        ...
        private void ugrdCuenta_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            try
            {
                if (ugrdCuenta.ActiveRow != null)
                {
                    ...
                    txt_Codigo.Text = (ugrdCuenta.ActiveRow.Cells[4].Value).ToString();
                    txtCodigoSic.Text = (ugrdCuenta.ActiveRow.Cells[4].Value).ToString();
                    txt_Nombre.Text = (ugrdCuenta.ActiveRow.Cells[5].Value).ToString();
                    txtValorNeto.Text = (ugrdCuenta.ActiveRow.Cells[6].Value).ToString();
                    txtCantidad.Text = (ugrdCuenta.ActiveRow.Cells[7].Value).ToString();
                    txtTotal.Text = (ugrdCuenta.ActiveRow.Cells[8].Value).ToString();
                    //txtCControl.Text = (ugrdCuenta.ActiveRow.Cells[14].Value).ToString();                   

                    dtpFechaPedido.Value = Convert.ToDateTime(ugrdCuenta.ActiveRow.Cells[3].Value);
                    txtPorcentaje.Text = "100";
                    CUENTAS_PACIENTES cuentas = new CUENTAS_PACIENTES();
        ...
        private void btnSolicitar_Click_1(object sender, EventArgs e)
        {
            ...
            if (txtCodMedico.Text != "")
            {
                ...
                {
                    x = new FrmAyudaAuditoriaProductos((PEDIDOS_AREAS)(cmb_Areas.SelectedItem), (RUBROS)(this.cmb_Rubros.SelectedItem), false, 0);
                ...
                //        else
                //        {
                //            valorAux = Convert.ToDecimal(fila.Cells["VALORUNITARIO"].Value.ToString()) * (Convert.ToInt16(fila.Cells["PORCENTAJE"].Value.ToString()));
        ...
        private void btnHonorarios_Click(object sender, EventArgs e)
        {
            if (txtCodMedico.Text != "")
            {
                frmCrearHonorarios honorario = new frmCrearHonorarios(Convert.ToInt32(txtCodigoPaciente.Text), Convert.ToInt32(txtCodMedico.Text));
