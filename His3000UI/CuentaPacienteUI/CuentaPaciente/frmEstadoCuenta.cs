    ...
    public partial class frmEstadoCuenta : Form
    {
        ...
        private void toolStripbtnNuevo_Click(object sender, EventArgs e)
        {
            ...
            if (this.txtPaquete.Text!="") 
            {
                ...
                /* Marca todos las lineas del grid */
        ...
        public void cargarGridPaquete()
        {
            ...
            DataTable datos = NegCuentasPacientes.AtencionesCuentasPaquete(Convert.ToInt64(this.txtPaquete.Text));          
        ...
        {
            DateTime fechaAltaIni1 =  dtpFiltroDesde.Value;
            DateTime fechaAltaFin1 =  dtpFiltroHasta.Value;

            if ((cmb_EstadoCuenta.Text) != "TODOS")
            {
                DataTable datos = NegCuentasPacientes.AtencionesCuentas_Paquetes
                (fechaAltaIni1, fechaAltaFin1, Convert.ToInt32(cmb_EstadoCuenta.SelectedValue.ToString()), this.txtPaciente.Text, this.txtHC.Text, txtAtencion.Text, Convert.ToInt16(this.cmbTipoCuenta.SelectedValue));
            ...
            {
                DataTable datos = NegCuentasPacientes.AtencionesCuentasTodosPaquete
                (fechaAltaIni1, fechaAltaFin1, Convert.ToInt32(cmb_EstadoCuenta.SelectedValue.ToString()), this.txtPaciente.Text, this.txtHC.Text, txtAtencion.Text,Convert.ToInt16 (this.cmbTipoCuenta.SelectedValue));
        ...
        private void toolStripButton4_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (var item in ugrdCuenta.Rows)
                {
                    ...
                    if ((bool)item.Cells["CHECKTRANS"].Value == true)
                    {
                         cadenaSql += Convert.ToString(item.Cells["ATENCION"].Value).Trim();
                        contador++;
                    }
                }
                if (contador > 0)
        ...
        private void toolStripButton7_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in ugrdCuenta.Rows)
            {
                ...
                if ((bool)item.Cells["CHECKTRANS"].Value == true)
                {
                    if (NegCuentasPacientes.VerificaAtencionesPaquete(Convert.ToInt32(item.Cells["ATE_CODIGO"].Value)) >0)
                    {
                        MessageBox.Show("La Atencion " + item.Cells["ATENCION"].Value.ToString() + " ya esta asignada a un paquete. Verifique por favor.");
                        return;
                    }

                    ListaAtencionesPaquete.Add(Convert.ToInt32(item.Cells["ATE_CODIGO"].Value));

                }
            }

            if (ListaAtencionesPaquete.Count > 0)
            {
                frmCreacionPaqueteCuentas frmCuentas = new frmCreacionPaqueteCuentas(ListaAtencionesPaquete,"NUEVO");
                frmCuentas.ShowDialog();
            }
        }

        private void MarcaLineasGrid()
        {
            int Fila = 0;
            ugrdCuenta.ActiveCell = ugrdCuenta.Rows[0].Cells["ATENCION"];
        ...
        private void MarcaLineasGrid_1()
        {
        ...
        private void toolStripButton8_Click(object sender, EventArgs e)
        {
            ...
            if (ListaAtencionesPaquete.Count > 0)
            {
                frmCreacionPaqueteCuentas frmCuentas = new frmCreacionPaqueteCuentas(ListaAtencionesPaquete, "MODIFICA");
                frmCuentas.ShowDialog();
            }
        ...
        private void toolStripButton9_Click(object sender, EventArgs e)
        {
                ...
                List<Int32> ListaAtencionesPaquete = new List<Int32>();
                frmCreacionPaqueteCuentas frmCuentas = new frmCreacionPaqueteCuentas(ListaAtencionesPaquete, "MODIFICA_NUMERO_TRAMITE");
                frmCuentas.ShowDialog();
        ...
        private void toolStripButton10_Click(object sender, EventArgs e)
        {
            ...
            if (ListaAtencionesPaquete.Count > 0)
            {
                frmCreacionPaqueteCuentas frmCuentas = new frmCreacionPaqueteCuentas(ListaAtencionesPaquete, "ELIMINAR");
                frmCuentas.ShowDialog();
            }
        }
        ...
        {
            List<Int32> ListaAtencionesPaquete = new List<Int32>();
            frmCreacionPaqueteCuentas frmCuentas = new frmCreacionPaqueteCuentas(ListaAtencionesPaquete, "ELIMINARP");
            frmCuentas.ShowDialog();
        }
        ...
        private void toolStripButton12_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in ugrdCuenta.Rows)
            {
                ...
                if ((bool)item.Cells["CHECKTRANS"].Value == true)
                {
                    Cadena += Convert.ToString(item.Cells["ATE_CODIGO"].Value).Trim();
                    contador++;
                }
            }
            ...
            if (contador > 1)
            {
                frmReporteDesgloseFactura Form = new frmReporteDesgloseFactura("", "", "", "", "", 0, "CUENTAS", Cadena, 0, "", "", 0, 0, "", "", "", "", "", "", "", "");
                Form.ShowDialog();
            }
        }
        ...
        private void btnPrefactura_Click(object sender, EventArgs e)
        {
            ...
            {                
                ...
                {
                    aux = 1;
                    His.Formulario.frmReportes frm = new His.Formulario.frmReportes(Convert.ToInt32(item.Cells["ATE_CODIGO"].Value), "PrefacturaRed");
                    frm.ShowDialog();
                    frm.Dispose();
                }
            }
            if (aux == 0)
            {
                MessageBox.Show("Marque una o varias atenciones :(", "HIS3000", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}
