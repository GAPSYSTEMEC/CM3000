...
namespace His.Datos
{
    public class DatTarifario
    {
        ...
        public DataTable Tarifario(string busqueda, bool codigo, bool descripcion)
        {
            ...
            if(busqueda == "")
            {
                command = new SqlCommand("SELECT TAD_CODIGO AS CODIGO, LTRIM(TAD_DESCRIPCION) AS DESCRIPCION FROM tarifario_detalle ORDER BY LTRIM(TAD_DESCRIPCION) ASC", connection);
            ...
            else
            {
                if (descripcion)
                {
                    command = new SqlCommand("SELECT TAD_CODIGO AS CODIGO, LTRIM(TAD_DESCRIPCION) AS DESCRIPCION FROM tarifario_detalle WHERE TAD_DESCRIPCION LIKE '%' + @filtro + '%' ORDER BY LTRIM(TAD_DESCRIPCION) ASC", connection);
                    command.CommandType = CommandType.Text;
                ...
                {
                    command = new SqlCommand("SELECT TAD_CODIGO AS CODIGO, LTRIM(TAD_DESCRIPCION) AS DESCRIPCION FROM tarifario_detalle WHERE TAD_CODIGO LIKE '%' + @filtro + '%' ORDER BY LTRIM(TAD_DESCRIPCION) ASC", connection);
                    command.CommandType = CommandType.Text;
                    command.Parameters.AddWithValue("@filtro", busqueda);
                    command.CommandTimeout = 180;
                    reader = command.ExecuteReader();
