    ...
    public class DatPacientes
    {
                ...
                if (tiponivel == "1")
                {
                    ...
                    foreach (var acceso in pacientes)
                    {
                        medicoc.Add(new DtoPacientesInfo()
                        {
                            ...
                            PAC_FECHA_CREACION = acceso.PAC_FECHA_CREACION,
        ...
        public void ActualizarPacienteAtencion(PACIENTES paciente)
        {
            using (var contexto = new HIS3000BDEntities(ConexionEntidades.ConexionEDM))
            {
                ...
                pacienteDestino.PAC_HISTORIA_CLINICA = paciente.PAC_HISTORIA_CLINICA;
        ...
        public DtoPacientes RecuperarDtoPacienteID(int codPaciente)
        {
            using (var contexto = new HIS3000BDEntities(ConexionEntidades.ConexionEDM))
            {
                ...
                if (pacientes != null)
                {
                    ...
                    paciente.PAC_FECHA_CREACION = pacientes.PAC_FECHA_CREACION;
        ...
        public PACIENTES RecuperarPacienteID(int codPaciente)
        {
            using (var contexto = new HIS3000BDEntities(ConexionEntidades.ConexionEDM))
            {
                return (from p in contexto.PACIENTES
                ...
                if (listaPacientes != null)
                {
                    foreach (var pacientes in listaPacientes)
                    {
                        dtoPacientes.Add(new DtoPacientes()
                        {
                            ...
                            PAC_FECHA_CREACION = pacientes.PAC_FECHA_CREACION,
        ...
        public DataTable getAtencionesIngresos(string desde, string hasta, bool ingreso, bool alta, bool facturacion, bool Fingreso, int Cod_Ingreso, bool Ftratamiento, int Cod_Tratamiento, bool Fhc, int hc, bool Festado)
        {
            SqlConnection Sqlcon;
            SqlCommand Sqlcmd;
            SqlDataAdapter Sqldap;
            ...
            Sqlcmd = new SqlCommand(" SELECT        A.ATE_CODIGO, A.HAB_CODIGO, H.hab_Numero, A.PAC_CODIGO, P.PAC_IDENTIFICACION, P.PAC_APELLIDO_PATERNO + SPACE(1) + P.PAC_APELLIDO_MATERNO + SPACE(1) + P.PAC_NOMBRE1 + SPACE(1)\n"
                        + "                             + P.PAC_NOMBRE2 AS NOMBRE_PACIENTE, P.PAC_HISTORIA_CLINICA, A.ATE_NUMERO_ATENCION, P.PAC_GENERO, CC.CAT_NOMBRE, A.TIF_OBSERVACION, CONVERT(int,ROUND(DATEDIFF(hour,P.PAC_FECHA_NACIMIENTO,A.ATE_FECHA_INGRESO)/8766.0,0)) AS EDAD, A.ATE_FECHA_INGRESO, A.ATE_FECHA_ALTA,\n"
                        + "                             A.ATE_FACTURA_FECHA, A.ATE_FACTURA_PACIENTE, A.MED_CODIGO, M.MED_APELLIDO_PATERNO + SPACE(1) + M.MED_APELLIDO_MATERNO + SPACE(1) + M.MED_NOMBRE1 + SPACE(1)\n"
                        + "                             + M.MED_NOMBRE2 AS NOMBRE_MEDICO, TT.TIA_DESCRIPCION, A.ATE_DIAGNOSTICO_INICIAL, U.APELLIDOS + SPACE(1) + U.NOMBRES AS NOMBRE_USUARIO, TIP.TIP_DESCRIPCION, ADS.ADS_ASEGURADO_NOMBRE,\n"
                        + "                             ADS.ADS_ASEGURADO_CEDULA, AIP.ANI_DESCRIPCION AS PARENTESCO, ADC.ADA_AUTORIZACION AS AUTORIZACION, AI.ANI_DESCRIPCION AS TIPO_SEGURO, AID.ANI_DESCRIPCION AS DEPENDENCIA,\n"
                         + "                            HE.HES_NOMBRE AS HABESTADO, REF.TIR_NOMBRE REFERIDO\n"
        ...
        public DataTable getAtencionesFormularios(string desde, string hasta, bool ingreso, bool alta, bool facturacion, bool Fingreso, int Cod_Ingreso, bool Ftratamiento, int Cod_Tratamiento, bool Fhc, int hc, bool Fformulario, string formulario)
        {
            SqlConnection Sqlcon;
            SqlCommand Sqlcmd;
            SqlDataAdapter Sqldap;
            ...
            Sqlcmd = new SqlCommand("SELECT A.ATE_FECHA_INGRESO AS 'FECHA INGRESO' ,F.TIPO,  A.PAC_HISTORIA_CLINICA AS HC, A.ATE_NUMERO_ATENCION AS 'NUMERO ATENCION',  A.NOMBRE_PACIENTE AS PACIENTE,   F.ID_Formulario, A.ATE_CODIGO, A.HAB_CODIGO, A.hab_Numero AS HAB,A.HABESTADO AS 'HAB ESTADO',A.PAC_CODIGO,A.PAC_IDENTIFICACION AS IDENTIFICACION,A.PAC_GENERO AS SEXO, A.ATE_DIAGNOSTICO_INICIAL AS 'DIAGNOSTICO INICIAL', A.MED_CODIGO,A.NOMBRE_MEDICO AS MEDICO,A.TIA_DESCRIPCION AS 'TIP. TRATAMIENTO', A.ATE_FECHA_ALTA AS 'FECHA ALTA', A.ATE_FACTURA_FECHA ,A.ATE_FACTURA_PACIENTE,A.NOMBRE_USUARIO AS USUARIO ,A.TIP_DESCRIPCION AS 'TIP. INGRESO',A.ADS_ASEGURADO_NOMBRE,A.ADS_ASEGURADO_CEDULA, A.PARENTESCO FROM \n"
                        + "     (\n"
                         + "           SELECT        ATE_CODIGO, ANE_CODIGO AS ID_Formulario, 'ANAMNESIS' AS TIPO FROM            dbo.HC_ANAMNESIS\n"
                        + "            UNION ALL\n"
                       + "             SELECT        ATE_CODIGO, EMER_CODIGO AS ID_Formulario, 'EMERGENCIA' AS TIPO  FROM            dbo.HC_EMERGENCIA_FORM\n"
                        + "            UNION ALL\n"
                        + "            SELECT        ATE_CODIGO, EPI_CODIGO AS ID_Formulario, 'EPICRISIS' AS TIPO  FROM            dbo.HC_EPICRISIS\n"
                        + "            UNION ALL\n"
                        + "            SELECT        ATE_CODIGO, EVO_CODIGO AS ID_Formulario, 'EVOLUCION' AS TIPO  FROM            dbo.HC_EVOLUCION\n"
                        + "            UNION ALL\n"
                        + "            SELECT        ATE_CODIGO, id_imagenologia AS ID_Formulario, 'IMAGENOLOGIA' AS TIPO  FROM            dbo.HC_IMAGENOLOGIA\n"
                        + "            UNION ALL\n"
                        + "            SELECT        ATE_CODIGO, HIN_CODIGO AS ID_Formulario, 'INTERCONSULTA' AS TIPO  FROM            dbo.HC_INTERCONSULTA\n"
                        + "            UNION ALL\n"
                        + "            SELECT        ATE_CODIGO, LCL_CODIGO AS ID_Formulario, 'LABORATORIO' AS TIPO  FROM            dbo.HC_LABORATORIO_CLINICO\n"
                        + "            UNION ALL\n"
                        + "            SELECT        ATE_CODIGO, ADF_CODIGO AS ID_Formulario, 'PROTOCOLO' AS TIPO  FROM            dbo.HC_PROTOCOLO_OPERATORIO\n"
                        + "              UNION ALL\n"
                        + "              SELECT AteCodigo, ID_FORM002, 'CONSULTA EXTERNA' AS TIPO FROM Form002MSP\n"
                        + "    ) F\n"
                        + "    JOIN(\n"
                        + "           SELECT        A.ATE_CODIGO, A.HAB_CODIGO, H.hab_Numero, A.PAC_CODIGO, P.PAC_IDENTIFICACION, P.PAC_APELLIDO_PATERNO + SPACE(1) + P.PAC_APELLIDO_MATERNO + SPACE(1) + P.PAC_NOMBRE1 + SPACE(1) \n"
                     + "+ P.PAC_NOMBRE2 AS NOMBRE_PACIENTE, P.PAC_HISTORIA_CLINICA, A.ATE_NUMERO_ATENCION, P.PAC_GENERO, A.TIF_OBSERVACION, A.ATE_FECHA_INGRESO, A.ATE_FECHA_ALTA, A.ATE_FACTURA_FECHA,\n"
