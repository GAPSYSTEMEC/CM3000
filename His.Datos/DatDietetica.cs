            ...
            switch (tabla)
            {
                ...
                case "KardexInsumos_CuentasProdsPorIngresar":
                    query = ("	DECLARE @ate_codigo VARCHAR(10)\n" +
                        ...
                        "from CUENTAS_PACIENTES where RUB_CODIGO = 27 and ATE_CODIGO = @ate_codigo) CUENTA WHERE CUE_CODIGO NOT IN(SELECT DISTINCT CUE_CODIGO FROM KARDEX_INSUMOS where ATE_CODIGO = @ate_codigo AND CUE_CODIGO <> 0) )");
                    break;
                case "KardexInsumos":
                ...
                case "Form022_Medicamentos":
                    query = ("select  distinct Presentacion,dosis, via,Frecuencia from KARDEXMEDICAMENTOS where atecodigo = " + codigo + " ");
                    break;
                case "Form022_Fechas":
                    query = ("select  distinct FechaAdministración from KARDEXMEDICAMENTOS where atecodigo = " + codigo + " ");
                    break;
                case "Form022_Registros":
                    query = ("select KARDEXMEDICAMENTOS.Hora, dbo.USUARIOS.NOMBRES,dbo.USUARIOS.APELLIDOS, DEPARTAMENTOS.DEP_NOMBRE, KARDEXMEDICAMENTOS.NoAdministrado,KARDEXMEDICAMENTOS.Observacion   FROM dbo.KARDEXMEDICAMENTOS left JOIN dbo.USUARIOS ON dbo.KARDEXMEDICAMENTOS.IDUSUARIO = dbo.USUARIOS.ID_USUARIO  INNER JOIN dbo.DEPARTAMENTOS ON dbo.USUARIOS.DEP_CODIGO = dbo.DEPARTAMENTOS.DEP_CODIGO\n" +
                ...
                case "DetalleEvolucionEnfermeria":
                    query = ("SELECT       dbo.HC_EVOLUCION_ENFERMERIA_DETALLE.EVD_CODIGO, dbo.HC_EVOLUCION_ENFERMERIA_DETALLE.ID_USUARIO , (dbo.HC_EVOLUCION_ENFERMERIA_DETALLE.NOM_USUARIO) as USUARIO, dbo.HC_EVOLUCION_ENFERMERIA_DETALLE.EVD_FECHA as FECHA, \n" +
                ...
                case "GetEgreso_DatosPaciente":
                    query = ("SELECT dbo.PACIENTES.PAC_HISTORIA_CLINICA, concat(dbo.PACIENTES.PAC_APELLIDO_PATERNO,' ', dbo.PACIENTES.PAC_APELLIDO_MATERNO,' ', dbo.PACIENTES.PAC_NOMBRE1,' ', dbo.PACIENTES.PAC_NOMBRE2) as PACIENTE, \n" +
...
"                         dbo.MEDICOS ON dbo.RETENCIONES_FUENTE.RET_CODIGO = dbo.MEDICOS.RET_CODIGO ON dbo.HONORARIOS_MEDICOS.MED_CODIGO = dbo.MEDICOS.MED_CODIGO\n" +
                            "where dbo.HONORARIOS_MEDICOS_DATOSADICIONALES.GENERADO_ASIENTO<>1 and HONORARIOS_MEDICOS.HOM_FACTURA_FECHA BETWEEN '" + codigo + "' and '" + codigo2 + "' AND (SELECT top 1 DEP_CODIGO FROM USUARIOS U INNER JOIN HONORARIOS_MEDICOS HM ON U.ID_USUARIO = HM.ID_USUARIO WHERE HM.ID_USUARIO = " +  His.Entidades.Clases.Sesion.codUsuario + ") <> 14 ");
                    }
                    else
                    {
                        query = ("SELECT 'false' as Seleccion, dbo.PACIENTES.PAC_HISTORIA_CLINICA as HC, dbo.ATENCIONES.ATE_CODIGO, \n" +
...
"                         dbo.MEDICOS ON dbo.RETENCIONES_FUENTE.RET_CODIGO = dbo.MEDICOS.RET_CODIGO ON dbo.HONORARIOS_MEDICOS.MED_CODIGO = dbo.MEDICOS.MED_CODIGO\n" +
 ...
 + " ORDER BY dbo.PEDIDOS.PED_FECHA DESC");
            ...
            bool facturacion, bool departamento, string coddep)
        {
            ...
            "CP.CUE_IVA AS IVA, ((CP.CUE_CANTIDAD * CUE_VALOR_UNITARIO) + CP.CUE_IVA) AS TOTAL, CP.Descuento AS DESCUENTO, CC.CAT_NOMBRE AS SEGURO, TR.TIR_NOMBRE AS REFERIDO\n" +
            ...
            "INNER JOIN TIPO_REFERIDO TR ON A.TIR_CODIGO = TR.TIR_CODIGO " + xWhere, connection);
