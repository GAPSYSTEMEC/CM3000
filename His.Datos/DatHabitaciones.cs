...
namespace His.Datos
{
    public class DatHabitaciones
    {
        ...
        public void CambiarEstadoHabitacion(HABITACIONES mHabitacion)
        {
            using (var contexto = new HIS3000BDEntities(ConexionEntidades.ConexionEDM))
            {
        ...
        public DataTable CambiaEstadoHabitacion(Int64 ateCodigo)
        {
            SqlConnection Sqlcon;
            SqlCommand Sqlcmd;
            SqlDataAdapter Sqldap;
        ...
        public List<HABITACIONES> listaHabitacionesEmergenciaLX()
        {
            using (var contexto = new HIS3000BDEntities(ConexionEntidades.ConexionEDM))
            {
                ...
                {
                    List<HABITACIONES> habitaciones = contexto.HABITACIONES.Include("NIVEL_PISO").Include("HABITACIONES_ESTADO").Where(h => h.hab_Activo == true && h.NIVEL_PISO.NIV_NOMBRE == "EMERGENCIA" || h.NIVEL_PISO.NIV_NOMBRE == "RESPIRATORIO").OrderBy(h => h.hab_Numero).ToList();
                    return habitaciones;
                }
                else
                {
        ...
        public void GuardarMedicosAlta(string medico, Int64 ate_codigo, string observacion, int usuario)
        {
            SqlCommand command;
            SqlConnection connection;
            BaseContextoDatos obj = new BaseContextoDatos();
