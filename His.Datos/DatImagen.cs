...
namespace His.Datos
{
    public class DatImagen
    {
        ...
        public void saveInformeAgendamiento(string[] p, List<PedidoImagen_diagnostico> x, Int64 id)
        {
            BaseContextoDatos obj = new BaseContextoDatos();
            SqlConnection Sqlcon = obj.ConectarBd();
            Sqlcon.Open();
            ...
            cadena_sql = "DECLARE @idnvo int; \n" +
                ...
                "    [PLACENTA_F],[PLACENTA_M],[PLACENTA_P],[MASCULINO],[FEMENINO],[MULTIPLE],[GRADO_MADUREZ],[ANTEVERSION],[RETROVERSION],[DIU],[FIBROMA],[MIOMA],[AUSENTE],[HIDROSALPIX],[QUISTE],[VACIA],[OCUPADA],[SACO_DOUGLAS],[recomendaciones],[PLACAS_ENVIADAS],[30X40],[8X10],[14X14],[14X17],[18X24],[ODONT],[DANADAS],[MEDIO_CONTRASTE])  \n" +
                "VALUES(@idnvo , " +
                p[0].ToString() + ", " +
                p[1].ToString() + ", " +
                p[2].ToString() + ", '" +
        ...
        public void guardarCambios(string[] p, List<PedidoImagen_diagnostico> x, Int64 id)
        {
            ...
            cadena_sql = "delete from[HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME] WHERE id_IMAGENOLOGIA_AGENDAMIENTO=" + id + "\n DECLARE @idnvo int; \n" +
                ...
                "[18X24],[ODONT],[DANADAS],[MEDIO_CONTRASTE])  \n" +
                "VALUES(@idnvo , " +
                p[0].ToString() + ", " +
                p[1].ToString() + ", " +
                p[2].ToString() + ", '" +
...
"WHERE dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME.id_IMAGENOLOGIA_AGENDAMIENTO = " + x + "  ", Sqlcon);
        ...
        "dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME.[18X24], dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME.ODONT, dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME.DANADAS, dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME.MEDIO_CONTRASTE, concat(TRIM(MEDICOS_2.MED_APELLIDO_PATERNO), ' ',TRIM(MEDICOS_2.MED_APELLIDO_MATERNO), ' ', TRIM(MEDICOS_2.MED_NOMBRE1), ' ', TRIM(MEDICOS_2.MED_NOMBRE2), '  CI:', SUBSTRING(MEDICOS_2.MED_RUC, 1,10)) as Tecnologo, concat(TRIM(MEDICOS_1.MED_APELLIDO_PATERNO), ' ', TRIM(MEDICOS_1.MED_APELLIDO_MATERNO), ' ',TRIM(MEDICOS_1.MED_NOMBRE1), ' ', TRIM(MEDICOS_1.MED_NOMBRE2), ' ', '  CI:', SUBSTRING(MEDICOS_1.MED_RUC, 1,10)) AS Radiologo\n" +
        ...
        public DataTable getAgendamientosInformes(string fechas)
        {
            ...
            //                          "where IA.estado = 1 and  IA.fecha_agendamiento  between " + fechas + " and  IAE.FECHA_REALIZADO >= A.ATE_FECHA_INGRESO \n " +
            //                          "order by IA.fecha_agendamiento asc", Sqlcon);
            //Sqlcmd = new SqlCommand("SELECT dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.id AS id, CONVERT(varchar, dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.fecha_agendamiento, 120) AS Fecha_Agendamiento, CONVERT(varchar, dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.fecha_solicitud, 120) AS Fecha_Solicitud, dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.ATE_CODIGO AS Atencion,\n" +
            //        "CONCAT(dbo.PACIENTES.PAC_APELLIDO_PATERNO, ' ', dbo.PACIENTES.PAC_APELLIDO_MATERNO, ' ', dbo.PACIENTES.PAC_NOMBRE1, ' ', dbo.PACIENTES.PAC_NOMBRE2) AS Paciente, dbo.PACIENTES.PAC_HISTORIA_CLINICA AS HC, dbo.HABITACIONES.hab_Numero as HAB,CONCAT(dbo.MEDICOS.MED_APELLIDO_PATERNO, ' ', dbo.MEDICOS.MED_NOMBRE1)  AS Radiologo, CONCAT(MEDICOS_1.MED_APELLIDO_PATERNO, ' ', MEDICOS_1.MED_NOMBRE1)  AS Tecnologo,\n" +
            //        "dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.observacion\n" +
            //        " FROM dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS INNER JOIN dbo.ATENCIONES ON dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.ATE_CODIGO = dbo.ATENCIONES.ATE_CODIGO INNER JOIN dbo.PACIENTES ON dbo.ATENCIONES.PAC_CODIGO = dbo.PACIENTES.PAC_CODIGO INNER JOIN dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME ON dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.id = dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_INFORME.id_IMAGENOLOGIA_AGENDAMIENTO INNER JOIN\n" +
            //             "dbo.HABITACIONES ON dbo.ATENCIONES.HAB_CODIGO = dbo.HABITACIONES.hab_Codigo LEFT OUTER JOIN dbo.MEDICOS ON dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.med_radiologo = dbo.MEDICOS.MED_CODIGO LEFT OUTER JOIN dbo.MEDICOS AS MEDICOS_1 ON dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.med_tecnologo = MEDICOS_1.MED_CODIGO\n" +
            //        "where [fecha_agendamiento] between " + fechas + " and  dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.estado = 1 order by dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS.fecha_agendamiento asc", Sqlcon);
            Sqlcmd.CommandType = CommandType.Text;
            Sqldap = new SqlDataAdapter();
            Sqlcmd.CommandTimeout = 180;
            Sqldap.SelectCommand = Sqlcmd;
        ...
        public DataTable getAgendamientoEstudios(string id_agendamiento)
        {
            ...
            Sqlcmd = new SqlCommand("SELECT dbo.CUENTAS_PACIENTES.CUE_CODIGO, dbo.PRODUCTO.PRO_DESCRIPCION FROM dbo.CUENTAS_PACIENTES INNER JOIN dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_ESTUDIOS ON dbo.CUENTAS_PACIENTES.CUE_CODIGO = dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_ESTUDIOS.CUE_CODIGO INNER JOIN dbo.PRODUCTO ON dbo.CUENTAS_PACIENTES.PRO_CODIGO = dbo.PRODUCTO.PRO_CODIGO  \n" +
                                    "WHERE dbo.HC_IMAGENOLOGIA_AGENDAMIENTOS_ESTUDIOS.id_imagenologia_agendamientos =" + id_agendamiento + " ", Sqlcon);
        ...
        public DataTable getReporteEncabezado(int idImagenologia)
        {
            ...
            Sqlcmd = new SqlCommand(" SELECT (SELECT TOP (1) EMP_NOMBRE FROM dbo.EMPRESA) AS clinica, dbo.PACIENTES_DATOS_ADICIONALES.COD_PARROQUIA, dbo.PACIENTES_DATOS_ADICIONALES.COD_CANTON, dbo.PACIENTES_DATOS_ADICIONALES.COD_PROVINCIA, dbo.PACIENTES.PAC_HISTORIA_CLINICA, dbo.PACIENTES.PAC_FECHA_NACIMIENTO,dbo.PACIENTES.PAC_APELLIDO_PATERNO, dbo.PACIENTES.PAC_APELLIDO_MATERNO, dbo.PACIENTES.PAC_NOMBRE1, dbo.PACIENTES.PAC_IDENTIFICACION, \n" +
                                     ...
                                     "dbo.HC_IMAGENOLOGIA.motivo, dbo.HC_IMAGENOLOGIA.resumen_clinico, dbo.HC_IMAGENOLOGIA.estado_movimiento, dbo.HC_IMAGENOLOGIA.estado_retirarsevendas, dbo.HC_IMAGENOLOGIA.estado_medicopresente, dbo.HC_IMAGENOLOGIA.estado_encama\n" +
        ...
        public DataTable getInterconsultas(int atecodigo)
        {
            SqlConnection Sqlcon;
            SqlCommand Sqlcmd;
            SqlDataAdapter Sqldap;
            ...
            Sqlcmd = new SqlCommand("SELECT HI.HIN_CODIGO AS ID, '[MEDICO: ' + HI.HIN_MED_INTERCONSULTADO + '] - [MOTIVO: ' + HI.HIN_DESCRIPCION_MOTIVO + ']' AS DATOS FROM HC_INTERCONSULTA HI WHERE HI.ATE_CODIGO = " + atecodigo, Sqlcon);
            Sqlcmd.CommandType = CommandType.Text;
            Sqldap = new SqlDataAdapter();
            Sqlcmd.CommandTimeout = 180;
            Sqldap.SelectCommand = Sqlcmd;
        ...
        public DataTable getProtocolos(Int64 ate_codigo)
        {
            ...
            Sqlcmd = new SqlCommand("SELECT PO.ADF_CODIGO AS ID, '[MED CIRUJANO: ' + PO.PROT_PROFESIONAL + '] - [MOTIVO: ' + PO.PROT_PREOPERATORIO + ']' AS DATOS  FROM HC_PROTOCOLO_OPERATORIO PO WHERE PO.ATE_CODIGO =" + ate_codigo, Sqlcon);
            Sqlcmd.CommandType = CommandType.Text;
            Sqldap = new SqlDataAdapter();
            Sqlcmd.CommandTimeout = 180;
            Sqldap.SelectCommand = Sqlcmd;
