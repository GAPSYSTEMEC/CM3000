...
namespace His.Datos
{
    public class DatRubros
    {
        ...
        public DataTable getCuentas(DateTime desde, DateTime hasta, bool ingreso, bool alta, string hc)
        {
            SqlCommand command;
            SqlDataReader reader;
            SqlConnection connection;
            ...
            command = new SqlCommand("SELECT CONCAT(P.PAC_APELLIDO_PATERNO, ' ', P.PAC_APELLIDO_MATERNO, ' ', P.PAC_NOMBRE1, ' ', P.PAC_NOMBRE2) AS PACIENTE, P.PAC_HISTORIA_CLINICA, " +
            "A.ATE_CODIGO CODIGO_UNICO_ATENCION, A.ATE_NUMERO_ATENCION, A.ATE_FECHA_INGRESO, A.ATE_FECHA_ALTA, isnull(dbo.f_EncuentraValores(A.ATE_CODIGO), 0) as VALOR, " +
            "CC.CAT_NOMBRE " +
            "FROM ATENCIONES A " +
