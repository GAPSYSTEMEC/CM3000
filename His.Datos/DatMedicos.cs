...
namespace His.Datos
{
    public class DatMedicos
    {
            ...
            using (var contexto = new HIS3000BDEntities(ConexionEntidades.ConexionEDM))
            {
                ...
                foreach (var acceso in med)
                {
                    ...
                    if (usuarios.Where(per => per.MEDICOS.MED_CODIGO == acceso.MED_CODIGO).FirstOrDefault() != null)
                        dtousu.Add(new DtoMedicos() { MED_CODIGO=acceso.MED_CODIGO, MED_APELLIDO_MATERNO=acceso.MED_APELLIDO_MATERNO,
                        MED_APELLIDO_PATERNO=acceso.MED_APELLIDO_PATERNO,MED_RUC=acceso.MED_RUC, ESP_NOMBRE=acceso.ESPECIALIDADES_MEDICAS.ESP_NOMBRE,
                        MED_NOMBRE1=acceso.MED_NOMBRE1,MED_NOMBRE2=acceso.MED_NOMBRE2});
                }
            ...
            using (var contexto = new HIS3000BDEntities(ConexionEntidades.ConexionEDM))
            {
                ...
                var medicos = (from m in contexto.MEDICOS
                                  ...
                                  // where m.MED_ESTADO ==true 
                                    select new {e.ESP_CODIGO,m.MED_AUTORIZACION_SRI,m.MED_APELLIDO_MATERNO,m.MED_APELLIDO_PATERNO
                                   ,m.MED_CODIGO,m.MED_CUENTA_CONTABLE,m.MED_DIRECCION,m.MED_ESTADO ,m.MED_FACTURA_FINAL,
                                   m.MED_FACTURA_INICIAL,m.MED_FECHA,m.MED_FECHA_NACIMIENTO,m.MED_FECHA_MODIFICACION ,
                                   m.MED_EMAIL,m.MED_GENERO,m.MED_NOMBRE1,m.MED_NOMBRE2,
                                   m.MED_NUM_CUENTA,r.RET_CODIGO,r.RET_DESCRIPCION,
                                                r.RET_PORCENTAJE,
                                                m.MED_RUC,
                                                m.MED_TELEFONO_CASA,
        ...
        public void GrabarMedico(MEDICOS medicoModificada, MEDICOS medicoOriginal, int esp_codigo, int tim_codigo, int tih_codigo)
        {
            ...
            command = new SqlCommand("UPDATE MEDICOS SET ESP_CODIGO = @esp_codigo, TIM_CODIGO = @tim_codigo, TIH_CODIGO = @tih_codigo WHERE MED_CODIGO = @med_codigo", connection);
        ...
        public DataTable getMedicosGrid()
        {
            ...
            Sqlcmd = new SqlCommand("SELECT dbo.MEDICOS.MED_CODIGO, concat(dbo.MEDICOS.MED_APELLIDO_PATERNO,' ', dbo.MEDICOS.MED_APELLIDO_MATERNO, ' ',dbo.MEDICOS.MED_NOMBRE1, ' ',dbo.MEDICOS.MED_NOMBRE2) as NOMBRE, \n" +
                                 "dbo.ESPECIALIDADES_MEDICAS.ESP_NOMBRE AS ESPECIALIDAD, dbo.MEDICOS.MED_DIRECCION AS DIRECCION, dbo.MEDICOS.MED_DIRECCION_CONSULTORIO AS DIRECCION_CONSULTORIO, dbo.MEDICOS.MED_RUC AS RUC, \n" +
